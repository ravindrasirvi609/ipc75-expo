...
export async function getPublicStates(): Promise<{ updatedAt: string; stalls: PublicStallState[] }> {
  const { data, error } = await supabaseAdmin.from("stall_bookings").select("stall_id,status,company,updated_at").order("stall_id");
...
export async function requestHold(request: HoldRequest): Promise<HoldResult> {
  ...
  const { data, error } = await supabaseAdmin.rpc("request_stall_hold", {
